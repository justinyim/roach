...
% Dancer calibration
%{
ts = readTelem('Data\2025.06.24_10.45.59_trial_imudata.txt');
toUse = [ts.femur(100:1300), ts.motor(100:1300)/25];
toUse(toUse(:,1) < -1.25,:) = [];
femur = toUse(:,1); crank = toUse(:,2);

ts = readTelem('Data\2025.07.03_17.02.27_trial_imudata.txt');
toUse = [ts.femur(60:1630), ts.motor(60:1630)/25];
toUse(toUse(:,1) < -1.25,:) = [];
femur = toUse(:,1); crank = toUse(:,2);
% Use curve fitter, 5th order polynomial

ts = readTelem('Data\2025.07.04_17.59.56_trial_imudata.txt');
toUse = [ts.femur(80:1670), ts.motor(80:1670)/25];
toUse(toUse(:,1) < -1.15,:) = [];
femur = toUse(:,1); crank = toUse(:,2);
% Use curve fitter, 5th order polynomial
%}
...
% Dancer parameters 6/24/2025 (kinematic) through 7/1/2025
%model(4).f_crank = @(x) -0.0845*x.^5 - 1.9195*x.^4 - 5.4056*x.^3 - 7.0471*x.^2 - 1.2509*x - 0.1460;
% Dancer parameters 7/1/2025 (kinematic) through 7/4/2025
%model(4).f_crank = @(x) -1.4806*x.^5 - 6.4726*x.^4 - 10.7775*x.^3 - 9.8908*x.^2 - 2.0553*x - 0.2126;
% Dancer parameters 7/4/2025 (kinematic) through 
model(4).f_crank = @(x) 2.3386*x.^5 + 5.0582*x.^4 + 2.3092*x.^3 - 2.6845*x.^2 + 0.4115*x - 0.0345;
%model(4).f_fd = @(x) -x.*(-0.0904)+x.^2.*0.0775-x.^3.*0.0468 + 0.09;
% Below copied from Dasher
model(4).f_fd = @(x) -x.*(-1.10783409238789e-1)+x.^2.*3.18398134512155e-2-x.^3.*2.62460134201421e-2 + 0.09;
f_MA_fit = @(x) (x.*7.868075191250353+x.^2.*6.052244719389666+x.^3.*6.708295328119813e-1-...
    x.^4.*1.126541544353222+2.219771122747685e-1)./...
    (x.*6.367962690243101e-2+x.^2.*7.873804026042631e-2-1.10783409238789e-1);
model(4).f_MA = @(x) (f_MA_fit(x)>1).*(f_MA_fit(x)-1) + 1;
model(4).f_MA_crank_foot = @(x) x.*(-2.499722373702723e2)+x.^2.*3.189439407444873e4-...
    x.^3.*2.631245153716756e6+x.^4.*8.301984361893313e7-x.^5.*1.32557409958358e9+...
    x.^6.*1.149347868439528e10-x.^7.*5.174523608827692e10+x.^8.*9.47419858088114e10-3.262662906298623;
model(4).f_spring = @(x,w) (x.*3.759274976416161e-1+x.^2.*4.275050419811049e-2).*(1 - 0.18*max(min(-0.05*w,1),-1));%-30*w,1),-1));
model(4).f_fric = @(x,w) (x.*3.759274976416161e-1+x.^2.*4.275050419811049e-2).*(0.18*max(min(-0.05*w,1),-1));%-30*w,1),-1));