...
extern long TObody_angle[3]; // yaw, roll, pitch
...
void takeoffEstimation(){
    ...
    int32_t TOcos_theta = (int32_t)cosApprox(TObody_angle[2]);
    int32_t TOsin_theta = (int32_t)cosApprox(TObody_angle[2]-PI/2);
    int32_t TOcos_phi = (int32_t)cosApprox(TObody_angle[1]);
    int32_t TOsin_phi = (int32_t)cosApprox(TObody_angle[1]-PI/2);
    int32_t TOcos_psi = (int32_t)cosApprox(TObody_angle[0]);
    int32_t TOsin_psi = (int32_t)cosApprox(TObody_angle[0]-PI/2);

    // Compensate for CG offset
    TObody_vel_LP[2] -= 0.4*TOlegVel*0.469; // (2^15/2000*180/pi)/2000 = 0.4694
    TObody_vel_LP[1] -= 0.1*TOlegVel*0.469;
    ...
    velocity[0] = (((((TOcos_psi*TOcos_theta) >> COS_PREC)
        ...
        + (((((TOcos_psi*TOsin_theta) >> COS_PREC)
            + ((((TOcos_theta*TOsin_psi) >> COS_PREC)*TOsin_phi) >> COS_PREC))*(int32_t)TOlegVel) >> COS_PREC);
    velocity[1] = (((((TOcos_theta*TOsin_psi) >> COS_PREC)
        ...
        + (((((TOsin_psi*TOsin_theta) >> COS_PREC)
            - ((((TOcos_psi*TOcos_theta) >> COS_PREC)*TOsin_phi) >> COS_PREC))*(int32_t)TOlegVel) >> COS_PREC);
    velocity[2] = - ((((TOcos_phi*TOsin_theta) >> COS_PREC)*vxw) >> COS_PREC)
        + ((TOsin_phi*vyw) >> COS_PREC)
        + ((((TOcos_theta*TOcos_phi) >> COS_PREC)*(int32_t)TOlegVel) >> COS_PREC);
